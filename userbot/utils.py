...
def load_module(shortname):
    if shortname.startswith("__"):
        ...
        LOGS.info("ZgBot - Successfully imported " + shortname)
    else:
        import userbot.utils
        ...
        mod.delete_zg = delete_zg
        mod.media_type = media_type
        # support for zgbot originals
        sys.modules["zgbot.utils"] = userbot.utils
        sys.modules["zgbot"] = userbot
        # support for paperplaneextended
        sys.modules["userbot.events"] = userbot.utils
        spec.loader.exec_module(mod)
        # for imports
        sys.modules["userbot.plugins." + shortname] = mod
        LOGS.info("⚡🔥  ʑɠ ცơɬ  Successfully Imported " + shortname)
    ...
    if pattern is not None:
        if pattern.startswith(r"\#"):
            ...
            if len(Config.COMMAND_HAND_LER) == 2:
                zgreg = "^" + Config.COMMAND_HAND_LER
                reg = Config.COMMAND_HAND_LER[1]
            elif len(Config.COMMAND_HAND_LER) == 1:
                zgreg = "^\\" + Config.COMMAND_HAND_LER
                reg = Config.COMMAND_HAND_LER
            args["pattern"] = re.compile(zgreg + pattern)
    ...
    if pattern is not None:
        if pattern.startswith(r"\#"):
            ...
            if len(Config.SUDO_COMMAND_HAND_LER) == 2:
                zgreg = "^" + Config.SUDO_COMMAND_HAND_LER
...
async def delete_zg(event, text, time=None, parse_mode=None, link_preview=None):
    parse_mode = parse_mode or "md"
    link_preview = link_preview or False
    time = time or 5
    if event.sender_id in Config.SUDO_USERS:
        ...
        zgevent = await event.edit(
            text, link_preview=link_preview, parse_mode=parse_mode
        )
    await asyncio.sleep(time)
    return await zgevent.delete()
...
async def unsavegif(event, Zazbhai):
    try:
        await event.client(
            functions.messages.SaveGifRequest(
                id=types.InputDocument(
                    id=Zazbhai.media.document.id,
                    access_hash=Zazbhai.media.document.access_hash,
                    file_reference=Zazbhai.media.document.file_reference,
                ),
                unsave=True,
            )
        )
