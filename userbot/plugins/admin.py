...
NO_ADMIN = "`I am not an admin Boss`"
...
async def promote(promt):
    ...
    zgevent = await edit_or_reply(promt, "Promoting...")
    ...
    try:
        ...
        await zgevent.edit("Promoted Successfully!")
    except BadRequestError:
        await zgevent.edit(NO_PERM)
        return
    if BOTLOG:
...
async def demote(dmod):
    ...
    if not admin and not creator:
        ...
        return
    zgevent = await edit_or_reply(dmod, "Demoting...")
    rank = "??????"
    ...
    try:
        ...
        await zgevent.edit(NO_PERM)
        return
    await zgevent.edit("Demoted Successfully!")
    if BOTLOG:
...
async def ban(bon):
    ...
    if not user:
        return
    zgevent = await edit_or_reply(bon, "Banning this retard")
    try:
    ...
    try:
        ...
        await zgevent.edit("I ain't got msg deleting right. But still Banned!")
        return
    if reason:
        await zgevent.edit(f"{str(user.id)} is banned !!\nReason: {reason}")
    else:
        await zgevent.edit(f"{str(user.id)} is banned!")
    if BOTLOG:
...
async def nothanos(unbon):
    ...
    if not admin and not creator:
        ...
        return
    zgevent = await edit_or_reply(unbon, "Unbanning...")
    ...
    try:
        ...
        await zgevent.edit("Unbanned Successfully!")
        if BOTLOG:
        ...
        await zgevent.edit("Sorry I Can't Unban This Retard!")
...
async def kick(usr):
    ...
    if not user:
        ...
        return
    zgevent = await edit_or_reply(usr, "Kicking...")
    try:
        await usr.client.kick_participant(usr.chat_id, user.id)
        await sleep(0.5)
    except Exception as e:
        await zgevent.edit(NO_PERM + f"\n{str(e)}")
        return
    if reason:
        await zgevent.edit(
        ...
        await zgevent.edit(f"Kicked [{user.first_name}](tg://user?id={user.id})!")
    if BOTLOG:
