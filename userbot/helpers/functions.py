...
async def admin_groups(zg):
    zggroups = []
    async for dialog in zg.client.iter_dialogs():
...
async def make_gif(event, file):
    ...
    async with event.client.conversation(chat) as conv:
        try:
            ...
            zgresponse = response if response.media else await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
            zgfile = await event.client.download_media(zgresponse, "./temp")
            return await unzip(zgfile)
...
async def iphonex(text):
    ...
    Zazbhai = r.get("message")
    zgurl = url(Zazbhai)
    if not zgurl:
        return "check syntax once more"
    with open("temp.png", "wb") as f:
        f.write(requests.get(Zazbhai).content)
...
async def waifutxt(text, chat_id, reply_to_id, bot, borg):
    ...
    zg = await sticcers[0].click("me", hide_via=True)
    if zg:
        await bot.send_file(int(chat_id), zg, reply_to=reply_to_id)
        await zg.delete()
